...
class InstallerModelInstall extends JModelLegacy
{
	...
	public function install()
	{
		...
		/*
		 * Check for a Webdata core package.
		...
		if (is_array($package) && isset($package['dir']) && is_dir($package['dir']))
		{
			...
			{
				// If a manifest isn't found at the source, this may be a Webdata package; check the package directory for the Webdata manifest
				if (file_exists($package['dir'] . '/administrator/manifests/files/webdata.xml'))
				{
					// We have a Webdata package
					if (in_array($installType, array('upload', 'url')))
					{
						JInstallerHelper::cleanupInstall($package['packagefile'], $package['extractdir']);
					}
					...
					$app->enqueueMessage(
						JText::sprintf('COM_INSTALLER_UNABLE_TO_INSTALL_JOOMLA_PACKAGE', JRoute::_('index.php?option=com_webdataupdate')),
						'warning'
					);
