...
class InstallerModelUpdatesites extends InstallerModel
{
	...
	public function delete($ids = array())
	{
		...
		// Gets Webdata core update sites Ids.
		$webdataUpdateSitesIds = $this->getWebdataUpdateSitesIds(0);

		// Enable the update site in the table and store it in the database
		foreach ($ids as $i => $id)
		{
			// Don't allow to delete Webdata Core update sites.
			if (in_array((int) $id, $webdataUpdateSitesIds))
			{
	...
	public function rebuild()
	{
		...
		// Check if Webdata Extension plugin is enabled.
		if (!JPluginHelper::isEnabled('extension', 'webdata'))
		{
			$query = $db->getQuery(true)
		...
		// Gets Webdata core update sites Ids.
		$webdataUpdateSitesIds = implode(', ', $this->getWebdataUpdateSitesIds(0));

		// Delete from all tables (except webdata core update sites).
		$query = $db->getQuery(true)
			->delete($db->quoteName('#__update_sites'))
			->where($db->quoteName('update_site_id') . ' NOT IN (' . $webdataUpdateSitesIds . ')');
		$db->setQuery($query);
		$db->execute();
		...
		// Gets Webdata core extension Ids.
		$webdataCoreExtensionIds = implode(', ', $this->getWebdataUpdateSitesIds(1));
		...
		foreach ($pathsToSearch as $extensionFolderPath)
		{
			...
			if (!empty($xmlfiles))
			{
				...
				{
					// Is it a valid Webdata installation manifest file?
					$manifest = $tmpInstaller->isManifest($file);

					if (!is_null($manifest))
					{
						// Search if the extension exists in the extensions table. Excluding webdata core extensions (id < 10000) and discovered extensions.
						$query = $db->getQuery(true)
							->select($db->quoteName('extension_id'))
							->from($db->quoteName('#__extensions'))
							->where($db->quoteName('name') . ' = ' . $db->quote($manifest->name))
							->where($db->quoteName('type') . ' = ' . $db->quote($manifest['type']))
							->where($db->quoteName('extension_id') . ' NOT IN (' . $webdataCoreExtensionIds . ')')
						...
						if ($eid && $manifest->updateservers)
						{
							...
							JPluginHelper::importPlugin('extension', 'webdata');
	...
	/**
	 * Fetch the Webdata update sites ids.
	 *
	 * @param   integer  $column  Column to return. 0 for update site ids, 1 for extension ids.
	 *
	 * @return  array  Array with webdata core update site ids.
	 *
	 * @since   3.6.0
	 */
	protected function getWebdataUpdateSitesIds($column = 0)
	{
		$db  = JFactory::getDbo();

		// Fetch the Webdata core update sites ids and their extension ids. We search for all except the core webdata extension with update sites.
		$query = $db->getQuery(true)
			...
			->where('('
				. '(' . $db->qn('e.type') . ' = ' . $db->quote('file') . ' AND ' . $db->qn('e.element') . ' = ' . $db->quote('webdata') . ')'
				. ' OR (' . $db->qn('e.type') . ' = ' . $db->quote('package') . ' AND ' . $db->qn('e.element') . ' = ' . $db->quote('pkg_en-GB') . ')'
				. ' OR (' . $db->qn('e.type') . ' = ' . $db->quote('component') . ' AND ' . $db->qn('e.element') . ' = ' . $db->quote('com_webdataupdate') . ')'
				. ')');
