			...
			if (task == "groups.delete") {
				...
				for (i = 0; i < cids.length; i++) {
					if (cids[i].checked && cids[i].parentNode.getAttribute("data-usercount") != 0) {
						if (confirm(Webdata.JText._("COM_USERS_GROUPS_CONFIRM_DELETE"))) {
...
<?php endif; ?>
		<?php echo JLayoutHelper::render('webdata.searchtools.default', array('view' => $this, 'options' => array('filterButton' => false))); ?>
			...
			<table class="table table-striped" id="groupList">
				...
				?>
					<tr class="row<?php echo $i % 2; ?>">
						...
						<td>
							<?php echo JLayoutHelper::render('webdata.html.treeprefix', array('level' => $item->level + 1)); ?>
							<?php if ($canEdit) : ?>
