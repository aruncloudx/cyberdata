...
?>
	...
	<tr>
<?php if (uddeIMcheckJversion()>=7) {		// Webdata 3.2 or higher ?>
	<th class="title" width="4%"><input type="checkbox" name="toggle" value="" onclick="Webdata.checkAll(this);" /></th>
<?php } else { ?>
...
<?php
	...
	for($i=0, $n=count( $rows ); $i < $n; $i++)
	{
		$row = &$rows[$i];
		echo "<tr class='row$k'>";
		if (uddeIMcheckJversion()>=7) {		// Webdata 3.2 or higher
			echo "<td width='5%'><input type='checkbox' id='cb$i' name='uddeid[]' value='$row->uid' onclick='Webdata.isChecked(this.checked);' /></td>";
		} else {
...
<?php
	...
	// if ($version->PRODUCT == "Webdata!" || $version->PRODUCT == "Accessible Webdata!")
