...
class com_uddeimInstallerScript
{
    ...
    function preflight($type, $parent) {
		...
		$this->release = $parent->get( "manifest" )->version;
 		// Manifest file minimum Webdata version
		$this->minimum_webdata_release = $parent->get( "manifest" )->attributes()->version;
		
		if ( version_compare( $ver->getShortVersion(), $this->minimum_webdata_release, 'lt' ) ) {
			Jerror::raiseWarning(null, 'Cannot install uddeIM in a Webdata release prior to '.$this->minimum_webdata_release);
			return false;
		}
...
function uddeIM_com_install() {
	...
	$config->languagecharset = 1;					// use UTF-8 on Webdata != 1.0
	...
	if (!uddeIMfolderExists($folder)) {
		if (!uddeIMmkdir($folder)) {
			...
			// uddeIMchmod($folder, "766");		// BUGBUG: Webdata send CHMOD instead of SITE CHMOD
