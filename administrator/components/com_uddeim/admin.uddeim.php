...
if (uddeIMcheckJversion()>=4) {	// Webdata >=2.5
...
if (uddeIMcheckJversion()>=5) {
	// load the admin css file for Webdata 3.0+
	$css = "";
...
//		$sql="SELECT id, toid, fromid, subject, message, datetime, stateto, inbox, outbox FROM `#__jam`".$limit;
//		$database->setQuery($sql);
//		$allpms=$database->loadObjectList();
//		foreach($allpms as $thepms) {
//			$toid			= $thepms->toid;
...
<?php	
	...
	if (uddeIMcheckJversion()>=5) {	// only Webdata 3.0, bug (not) fixed in Webdata 3.1
		...
		<table width="100%" border="0" cellpadding="4" cellspacing="2" class="adminForm" id="adminForm">
			<tr align="center" valign="middle">
				<td align="left" valign="top">
					...
					PMS component for Mambo and Webdata<br />
...
// when this is removed, deleted users are shown as "User deleted" in the inbox
	...
	// not really a database fix but helpful for existing Webdata 1.0 installations
...
function uddeIMmaintenanceCheckFix($option, $task, $act, $config) {
	...
	// Webdata 1.5 installer fix, this ensures that Webdata 1.5 finds the correct XML file and the extension manager does not claim incompatibility
	$mod1 = 0;
	$mod2 = 0;
	$mod3 = 0;
	$mod4 = 0;
