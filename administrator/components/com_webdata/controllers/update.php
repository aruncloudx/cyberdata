...
/**
 * The Webdata! update controller for the Update view
 *
 * @since  2.5.4
 */
class WebdataupdateControllerUpdate extends JControllerLegacy
{
	/**
	...
	public function download()
	{
		...
		/** @var WebdataupdateModelDefault $model */
		...
		if ($file)
		{
			JFactory::getApplication()->setUserState('com_webdataupdate.file', $file);
			$url = 'index.php?option=com_webdataupdate&task=update.install&' . JFactory::getSession()->getFormToken() . '=1';
		...
		else
		{
			...
			$url = 'index.php?option=com_webdataupdate';
	...
	/**
	 * Start the installation of the new Webdata! version
	 *
	 * @return  void
	 *
	 * @since   2.5.4
	...
	public function finalise()
	{
		...
		{
			$this->setRedirect('index.php?option=com_webdataupdate&view=update&layout=finaliseconfirm');
		...
		$url = 'index.php?option=com_webdataupdate&task=update.cleanup&' . JFactory::getSession()->getFormToken() . '=1';
	...
	public function cleanup()
	{
		...
		$url = 'index.php?option=com_webdataupdate&view=default&layout=complete';
		$this->setRedirect($url);
	...
	public function upload()
	{
		...
		$url = 'index.php?option=com_webdataupdate&task=update.captive&' . $token . '=1';
		$this->setRedirect($url);
	}
	...
	public function captive()
	{
		...
		$tempFile = JFactory::getApplication()->getUserState('com_webdataupdate.temp_file', null);
	...
	public function confirm()
	{
		...
		JFactory::getApplication()->setUserState('com_webdataupdate.file', basename($tempFile));
	 ...
	 *
	 * @return  WebdataupdateControllerUpdate  This object to support chaining.
	 *
	 * @since   2.5.4
	 */
	...
	/**
	 * Applies FTP credentials to Webdata! itself, when required
	 *
	 * @return  void
	 *
	 * @since   2.5.4
	...
	{
		JFactory::getApplication()->getUserStateFromRequest('com_webdataupdate.method', 'method', 'direct', 'cmd');

		if (!JClientHelper::hasCredentials('ftp'))
		{
			$user = JFactory::getApplication()->getUserStateFromRequest('com_webdataupdate.ftp_user', 'ftp_user', null, 'raw');
			$pass = JFactory::getApplication()->getUserStateFromRequest('com_webdataupdate.ftp_pass', 'ftp_pass', null, 'raw');
	...
	public function finaliseconfirm()
	{
		...
		$this->setRedirect('index.php?option=com_webdataupdate&task=update.finalise&' . JFactory::getSession()->getFormToken() . '=1');
	}
}
