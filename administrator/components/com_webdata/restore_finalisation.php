...
// Fake a miniature Webdata environment
if (!defined('_JEXEC'))
{
	define('_JEXEC', 1);
}
...
{
	/**
	 * Run part of the Webdata! finalisation script, namely the part that cleans up unused files/folders
	 *
	 * @param   string  $siteRoot     The root to the Webdata! site
	...
	function finalizeRestore($siteRoot, $restorePath)
	{
		...
		// Make sure Webdata!'s code can figure out which files exist and need be removed
		clearstatcache();

		// Remove obsolete files - prevents errors occuring in some system plugins
		if (class_exists('WebdataInstallerScript'))
		{
			$script = new WebdataInstallerScript;
