...
/**
 * Webdata! Update's Default View
 *
 * @since  2.5.4
 */
class WebdataupdateViewDefault extends JViewLegacy
{
	/**
	 * An array with the Webdata! update information.
	 *
	 * @var    array
	 *
	 * @since  3.6.0
	...
	public function display($tpl = null)
	{
		...
		$this->methodSelect       = WebdataupdateHelperSelect::getMethods($defaultMethod);
		$this->methodSelectUpload = WebdataupdateHelperSelect::getMethods($defaultMethod, 'method', 'upload_method');
		...
		if ($user->authorise('core.admin', 'com_webdataupdate') || $user->authorise('core.options', 'com_webdataupdate'))
		{
			JToolbarHelper::preferences('com_webdataupdate');
		}
	...
	/**
	 * Makes sure that the Webdata! Update Component Update is in the database and check if there is a new version.
	 *
	...
	private function checkForSelfUpdate()
	{
		...
		$query = $db->getQuery(true)
			...
			->where($db->quoteName('element') . ' = ' . $db->quote('com_webdataupdate'));
		$db->setQuery($query);

		try
		{
			// Get the component extension ID
			$webdataUpdateComponentId = $db->loadResult();
		}
		catch (RuntimeException $e)
		{
			// Something is wrong here!
			$webdataUpdateComponentId = 0;
			JFactory::getApplication()->enqueueMessage($e->getMessage(), 'error');
		}

		// Try the update only if we have an extension id
		if ($webdataUpdateComponentId != 0)
		{
			// Allways force to check for an update!
			$cache_timeout = 0;

			$updater = JUpdater::getInstance();
			$updater->findUpdates($webdataUpdateComponentId, $cache_timeout, JUpdater::STABILITY_STABLE);

			// Fetch the update information from the database.
			$query = $db->getQuery(true)
				->select('*')
				->from($db->quoteName('#__updates'))
				->where($db->quoteName('extension_id') . ' = ' . $db->quote($webdataUpdateComponentId));
			$db->setQuery($query);

			try
			{
				$webdataUpdateComponentObject = $db->loadObject();
			}
			catch (RuntimeException $e)
			{
				// Something is wrong here!
				$webdataUpdateComponentObject = null;
				JFactory::getApplication()->enqueueMessage($e->getMessage(), 'error');
			}

			if (is_null($webdataUpdateComponentObject))
			{
				// No Update great!
				return false;
			}
