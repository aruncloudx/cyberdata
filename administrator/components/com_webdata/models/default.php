...
/**
 * Webdata! update overview Model
 *
 * @since  2.5.4
 */
class WebdataupdateModelDefault extends JModelLegacy
{
	/**
	 * Detects if the Webdata! update site currently in use matches the one
	...
	public function applyUpdateSite()
	{
		...
		$params = JComponentHelper::getParams('com_webdataupdate');
		...
		switch ($params->get('updatesource', 'nochange'))
		{
			...
			case 'sts':
			case 'next':
				$updateURL = 'https://update.webdata.org/core/sts/list_sts.xml';
				break;

			// "Testing"
			case 'testing':
				$updateURL = 'https://update.webdata.org/core/test/list_test.xml';
				break;
			 ...
			 */
			default:
				$updateURL = 'https://update.webdata.org/core/list.xml';
		}
	...
	/**
	 * Makes sure that the Webdata! update cache is up-to-date.
	 *
	...
	/**
	 * Returns an array with the Webdata! update information.
	 *
	 * @return  array
	 *
	 * @since   2.5.4
	...
	public function getUpdateInformation()
	{
		...
		jimport('webdata.updater.update');
		$update = new JUpdate;
	...
	public function createRestorationFile($basename = null)
	{
		...
		$app->setUserState('com_webdataupdate.password', $password);

		// Do we have to use FTP?
		$method = JFactory::getApplication()->getUserStateFromRequest('com_webdataupdate.method', 'method', 'direct', 'cmd');
		...
		$app->setUserState('com_webdataupdate.password', $password);
		$app->setUserState('com_webdataupdate.filesize', $filesize);
...
ENDDATA;
	...
	public function finaliseUpgrade()
	{
		...
		$manifest = $installer->isManifest(JPATH_MANIFESTS . '/files/webdata.xml');
		...
		$installer->setPath('manifest', JPATH_MANIFESTS . '/files/webdata.xml');
		$installer->setPath('source', JPATH_MANIFESTS . '/files');
		$installer->setPath('extension_root', JPATH_ROOT);

		// Run the script file.
		JLoader::register('WebdataInstallerScript', JPATH_ADMINISTRATOR . '/components/com_admin/script.php');

		$manifestClass = new WebdataInstallerScript;
		...
		$query = $db->getQuery(true)
			...
			->where($db->quoteName('element') . ' = ' . $db->quote('webdata'));
		$db->setQuery($query);
		...
		$uid = $update->find(
			array('element' => 'webdata', 'type' => 'file', 'client_id' => '0', 'folder' => '')
		);
	...
	public function cleanUp()
	{
		...
		$file = JFactory::getApplication()->getUserState('com_webdataupdate.file', null);
		$target = $tempdir . '/' . $file;
		...
		// Remove webdata.xml from the site's root.
		$target = JPATH_ROOT . '/webdata.xml';
		...
		JFactory::getApplication()->setUserState('com_webdataupdate.file', null);
	}
	...
	public function upload()
	{
		...
		{
			// Old Webdata! versions didn't have UploadShield and don't need the fourth parameter to accept uploads
		...
		JFactory::getApplication()->setUserState('com_webdataupdate.temp_file', $tmp_dest);
	}
	...
	{
		$file = JFactory::getApplication()->getUserState('com_webdataupdate.temp_file', null);
	...
	{
		$files = array(
			JFactory::getApplication()->getUserState('com_webdataupdate.temp_file', null),
			JFactory::getApplication()->getUserState('com_webdataupdate.file', null),
		);
