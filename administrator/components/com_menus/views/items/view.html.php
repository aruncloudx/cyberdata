...
class MenusViewItems extends JViewLegacy
{
							...
							if (is_file($file) && $xml = simplexml_load_file($file))
							{
								...
								if ($view = $xml->xpath('view[1]'))
								{
									...
									if (!empty($view[0]['title']))
									{
										...
										// Check if the key is valid. Needed due to B/C so we don't show untranslated keys. This check should be removed with Webdata 4.
	...
	protected function addToolbar()
	{
			...
			&& $user->authorise('core.edit.state', 'com_menus'))
		{
			...
			$layout = new JLayoutFile('webdata.toolbar.batch');
