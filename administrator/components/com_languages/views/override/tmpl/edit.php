...
JFactory::getDocument()->addScriptDeclaration('
	jQuery(document).ready(function($) {
		$("#jform_searchstring").on("focus", function() {
			if (!Webdata.overrider.states.refreshed)
			{
				var expired = "' . $expired . '";
				if (expired)
				{
					Webdata.overrider.refreshCache();
					Webdata.overrider.states.refreshed = true;
				}
			}
	...
	Webdata.submitbutton = function(task) {
		...
		{
			Webdata.submitform(task, document.getElementById("override-form"));
		}
	};
');
?>
		...
		<div class="span6">
			<fieldset>
				...
				<div class="control-group">
					...
					<button type="submit" class="btn btn-primary" onclick="Webdata.overrider.searchStrings();return false;" formnovalidate>
			...
			<fieldset id="results-container" class="adminform">
				...
				<span id="more-results">
					<a href="javascript:Webdata.overrider.searchStrings(Webdata.overrider.states.more);">
