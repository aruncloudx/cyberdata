...
function pagination_list_footer($list)
{
	 ...
	 *
	 * Remove the onchange=Webdata.submitform from the select tag
	 * Add in a button with onclick instead
	 */
	$fixlimit = $list['limitfield'];
	$fixlimit = preg_replace('/onchange="Webdata.submitform\(\);"/', '', $fixlimit);

	$html = '<div class="containerpg"><div class="pagination">';

	$html .= '<div class="limit"><label for="limit">' . JText::_('JGLOBAL_DISPLAY_NUM') . ' </label>';
	$html .= "\n" . $fixlimit;
	$html .= "\n" . '<button id="pagination-go" type="button" onclick="Webdata.submitform()">' . JText::_('JSUBMIT') . '</button></div>';
	$html .= "\n" . $list['pageslinks'];
...
function pagination_item_active(&$item)
{
	if ($item->base > 0)
	{
		return '<a href="#" title="'.$item->text.'" onclick="document.adminForm.' . $item->prefix . 'limitstart.value=' .$item->base.'; Webdata.submitform();return false;">'.$item->text. '</a>';
	}
	else
	{
		return '<a href="#" title="'.$item->text.'" onclick="document.adminForm.' . $item->prefix . 'limitstart.value=0; Webdata.submitform();return false;">'.$item->text. '</a>';
	}
}
