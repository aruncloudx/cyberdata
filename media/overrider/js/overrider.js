 ...
 * @since   2.5
 */
Webdata.overrider.refreshCache = function()
{
	...
	}).done(function (r)
	{
		...
		if (r.messages)
		{
			Webdata.renderMessages(r.messages);
		}
	...
	{
		alert(Webdata.JText._('COM_LANGUAGES_VIEW_OVERRIDE_REQUEST_ERROR'));
		$('#refresh-status').slideUp().hide();
	});
};
 ...
 * @since   2.5
 */
Webdata.overrider.searchStrings = function(more)
{
	var $ = jQuery.noConflict(), self = this;
 ...
 * @since   2.5
 */
Webdata.overrider.insertResults = function(results)
{
	var $ = jQuery.noConflict(), self = this;
	...
	$.each(results, function(index, item) {
		...
		var $div = $('<div>', {
			...
			onclick: 'Webdata.overrider.selectString(' + self.states.counter + index + ');'
		});
	...
	{
		var $noresult = $('<div>',{
			html: Webdata.JText._('COM_LANGUAGES_VIEW_OVERRIDE_NO_RESULTS')
		});
 ...
 * @since   2.5
 */
Webdata.overrider.selectString = function(id)
{
