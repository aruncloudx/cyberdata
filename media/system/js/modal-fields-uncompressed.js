...
(function() {
	...
	window.processModalEdit = function (element, fieldPrefix, action, itemType, task, formId, idFieldId, titleFieldId)
	{
		...
		if (task === 'cancel')
		{
			...
			document.getElementById('Frame_' + modalId).contentWindow.Webdata.submitbutton(itemType.toLowerCase() + '.' + task);
		...
		else
		{
			...
			if (iframeDocument.formvalidator.isValid(iframeDocument.getElementById(formId)))
			{
				...
				document.getElementById('Frame_' + modalId).contentWindow.Webdata.submitbutton(itemType.toLowerCase() + '.' + submittedTask);
			}
		}
