...
window.showAssociationMessage = function()
{
	...
	jQuery('#associations').prepend('<div id="associations-notice" class="alert alert-info">' + Webdata.JText._('JGLOBAL_ASSOC_NOT_POSSIBLE') + '</div>');
}
...
!(function()
{
	...
	{	
		var associationsEditOptions = Webdata.getOptions('system.associations.edit'), formControl = associationsEditOptions.formControl || 'jform';
		...
		$('#' + formControl + '_language').on('change', function(event)
		{
			...
			{
				Webdata.renderMessages({warning: [Webdata.JText._('JGLOBAL_ASSOCIATIONS_RESET_WARNING')]});
			}
