...
Date.numbersToIso = function(str) {
	...
	if (Object.prototype.toString.call(WebdataCalLocale.localLangNumbers) === '[object Array]') {
		for (i = 0; i < nums.length; i++) {
			str = str.replace(new RegExp(WebdataCalLocale.localLangNumbers[i], 'g'), nums[i]);
		}
	}
	return str;
};
...
Date.prototype.print = function (str, dateType, translate) {
	...
	s["%a"] = WebdataCalLocale.shortDays[w];                                                     // abbreviated weekday name
	s["%A"] = WebdataCalLocale.days[w];                                                          // full weekday name
	s["%b"] = WebdataCalLocale.shortMonths[m];                                                   // abbreviated month name
	s["%B"] = WebdataCalLocale.months[m];                                                        // full month name
	...
	s["%n"] = "\n";                                                                             // a newline character
	s["%p"] = pm ? WebdataCalLocale.PM : WebdataCalLocale.AM;
	s["%P"] = pm ? WebdataCalLocale.pm : WebdataCalLocale.am;
	...
	if (Object.prototype.toString.call(WebdataCalLocale.localLangNumbers) === '[object Array]' && translate)
...
Date.parseFieldDate = function(str, fmt, dateType) {
	...
	for (i = 0; i < a.length; ++i) {
		...
		switch (b[i]) {
			...
			case "%B":
				for (j = 0; j < 12; ++j) {
					if (dateType != 'gregorian') {
						if (WebdataCalLocale.months[j].substr(0, a[i].length).toLowerCase() == a[i].toLowerCase()) { m = j; break; }
					} else {
						if (WebdataCalLocale.months[j].substr(0, a[i].length).toLowerCase() == a[i].toLowerCase()) { m = j; break; }
					}
				}
	...
	for (i = 0; i < a.length; ++i) {
		if (a[i].search(/[a-zA-Z]+/) != -1) {
			...
			for (j = 0; j < 12; ++j) {
				if (dateType != 'gregorian') {
					if (WebdataCalLocale.months[j].substr(0, a[i].length).toLowerCase() == a[i].toLowerCase()) { t = j; break; }
				} else {
					if (WebdataCalLocale.months[j].substr(0, a[i].length).toLowerCase() == a[i].toLowerCase()) { t = j; break; }
				}
			}
