...
jQuery(document).ready(function($) {
	...
	// Save button actions, replacing the default Webdata.submitbutton() with custom function.
	Webdata.submitbutton = function(task)
	{
		...
		else if(task === 'copy')
		{
			...
			window.frames['reference-association'].Webdata.submitbutton(document.getElementById('adminForm').getAttribute('data-associatedview') + '.save2copy');
		}
		// Undo association
		else if (task === 'undo-association')
		{
			...
			Webdata.submitbutton('reference');
		}
		...
		else
		{
			...
			window.frames[task + '-association'].Webdata.submitbutton(document.getElementById('adminForm').getAttribute('data-associatedview') + '.apply');
		}

		return false;
	};

	// Preload Webdata loading layer.
	Webdata.loadingLayer('load');
	...
	$(document).on('change', '#jform_itemlanguage', function() {
		...
		if (selected !== '' && typeof selected !== 'undefined')
		{
			...
			// Iframe load start, show Webdata loading layer.
			Webdata.loadingLayer('show');
	...
	$('#reference-association').on('load', function() {
		...
		// Iframe load finished, hide Webdata loading layer.
		Webdata.loadingLayer('hide');
	});
