var FinderIndexer = function(){
	...
	var handleResponse = function (json, resp) {
		try {
			...
			try {
				...
				if (error === '') {
					error = Webdata.JText._('COM_FINDER_NO_ERROR_RETURNED');
				}
				jQuery('#finder-progress-header').text(Webdata.JText._('COM_FINDER_AN_ERROR_HAS_OCCURRED')).addClass('finder-error');
	...
	var handleFailure= function (xhr) {
		...
		var header = json ? json.header : Webdata.JText._('COM_FINDER_AN_ERROR_HAS_OCCURRED');
		var message = json ? json.message : Webdata.JText._('COM_FINDER_MESSAGE_RETURNED') + ' <br />' + json;
