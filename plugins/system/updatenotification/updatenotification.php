...
/**
 * Webdata! Update Notification plugin
 *
 * Sends out an email to all Super Users or a predefined email address when a new Webdata! version is available.
 *
 ...
 * GPLv3 or later) to allow its inclusion in the Webdata! CMS.
 *
 * @since  3.5
 */
class PlgSystemUpdatenotification extends JPlugin
{
	...
	{
		// Get the timeout for Webdata! updates, as configured in com_installer's component parameters
		JLoader::import('webdata.application.component.helper');
		$component = JComponentHelper::getComponent('com_installer');

		/** @var \Webdata\Registry\Registry $params */
		...
		// This is the extension ID for Webdata! itself
		$eid = 700;
		...
		// Unfortunately Webdata! MVC doesn't allow us to autoload classes
		JModelLegacy::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_installer/models', 'InstallerModel');

		// Get the update model and retrieve the Webdata! core updates
		...
		// If we're here, we have updates. First, get a link to the Webdata! Update component.
		$baseURL  = JUri::base();
		$baseURL  = rtrim($baseURL, '/');
		$baseURL .= (substr($baseURL, -13) !== 'administrator') ? '/administrator/' : '/';
		$baseURL .= 'index.php?option=com_webdataupdate';
		 ...
		 * site, confusing them (they can't understand the third party security solution is not part of Webdata! proper).
		 ...
		 * update email which would make Webdata! seem bad. So, please, if you don't fully understand what the
		...
		$substitutions = array(
			...
			'[URL]'         => JUri::base(),
			'[LINK]'        => $uri->toString(),
			'[RELEASENEWS]' => 'https://www.webdata.org/announcements/release-news/',
			'\\n'           => "\n",
		);
