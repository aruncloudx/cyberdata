...
class PlgEditorTinymce extends JPlugin
{
	...
	{
		return 'Webdata.editors.instances[' . json_encode($id) . '].getValue();';
	}
	...
	{
		return 'Webdata.editors.instances[' . json_encode($id) . '].setValue(' . json_encode($html) . ');';
	}
		...
		$name, $content, $width, $height, $col, $row, $buttons = true, $id = null, $asset = null, $author = null, $params = array())
	{
		...
		$textarea->name    = $name;
		$textarea->id      = $id;
		$textarea->class   = 'mce_editable webdata-editor-tinymce';
		$textarea->cols    = $col;
		$textarea->rows    = $row;
		...
		$editor .= JLayoutHelper::render('webdata.tinymce.textarea', $textarea);
		$editor .= $this->_toogleButton($id);
		$editor .= '</div>';

		// Prepare the instance specific options, actually the ext-buttons
		if (empty($options['tinyMCE'][$fieldName]['webdataExtButtons']))
		{
			...
			$options['tinyMCE'][$fieldName]['webdataMergeDefaults'] = true;
			$options['tinyMCE'][$fieldName]['webdataExtButtons']    = $btns;
		...
		$levelParams      = new Webdata\Registry\Registry;
		...
		{
			// Note this check for the template_list.js file will be removed in Webdata 4.0
		...
		if ($dragdrop && $user->authorise('core.create', 'com_media'))
		{
			...
			// Is Webdata installed in subdirectory
			if (JUri::root(true) !== '/')
			{
	...
	{
		return JLayoutHelper::render('webdata.tinymce.togglebutton', $name);
	}
