...
class PlgEditorCodemirror extends JPlugin
{
	...
	{
		return sprintf('document.getElementById(%1$s).value = Webdata.editors.instances[%1$s].getValue();', json_encode((string) $id));
	}
	...
	{
		return sprintf('Webdata.editors.instances[%1$s].getValue();', json_encode((string) $id));
	}
	...
	{
		return sprintf('Webdata.editors.instances[%1$s].setValue(%2$s);', json_encode((string) $id), json_encode((string) $content));
	}
	...
	public function onGetInsertMethod()
	{
		...
		;function jInsertEditorText(text, editor) { Webdata.editors.instances[editor].replaceSelection(text); }
		");
	...
	protected function displayButtons($name, $buttons, $asset, $author)
	{
		...
		if (is_array($buttons) || (is_bool($buttons) && $buttons))
		{
			...
			$return .= JLayoutHelper::render('webdata.editors.buttons', $buttons);
		}
