...
abstract class JLoader
{
	...
	public static function import($key, $base = null)
	{
		...
		if (!isset(self::$imported[$key]))
		{
			...
			// If we are importing a library from the Webdata namespace set the class to autoload.
			if (strpos($path, 'webdata') === 0)
			{
				// Since we are in the Webdata namespace prepend the classname with J.
				$class = 'J' . $class;
			...
			/*
			 * If we are not importing a library from the Webdata namespace directly include the
	...
	/**
	 * Method to setup the autoloaders for the Webdata Platform.
	 ...
	 * @param   boolean  $enablePrefixes  True to enable prefix based class loading (needed to auto load the Webdata core).
	 * @param   boolean  $enableClasses   True to enable class map based class loading (needed to auto load the Webdata core).
	 *
	 * @return  void
	 *
	 * @since   12.3
	...
	public static function setup($enablePsr = true, $enablePrefixes = true, $enableClasses = true)
	{
		...
		if ($enablePrefixes)
		{
			// Register the J prefix and base path for Webdata platform libraries.
			self::registerPrefix('J', JPATH_PLATFORM . '/webdata');
