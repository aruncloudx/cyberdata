...
/**
 * Webdata! Stream Interface
 *
 * The Webdata! stream interface is designed to handle files as streams
...
class JStream extends JObject
{
		...
		$use_prefix = false, $relative = false, $detectprocessingmode = false)
	{
		...
		if (isset($url['scheme']))
		{
			// If we're dealing with a Webdata! stream, load it
			if (JFilesystemHelper::isWebdataStream($url['scheme']))
			{
