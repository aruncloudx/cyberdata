...
JLoader::import('webdata.filesystem.folder');
JLoader::import('webdata.filesystem.file');
JLoader::import('webdata.installer.installer');
JLoader::import('webdata.utilities.date');
...
abstract class FOFUtilsInstallscript
{
	...
	/**
	 * The minimum Webdata! version required to install this extension
	 *
	 * @var   string
	 */
	protected $minimumWebdataVersion = '2.5.6';

	/**
	 * The maximum Webdata! version this extension can be installed on
	 *
	 * @var   string
	 */
	protected $maximumWebdataVersion = '3.9.99';
	...
	/**
	 * Post-installation message definitions for Webdata! 3.2 or later.
	 *
	 * This array contains the message definitions for the Post-installation Messages component added in Webdata! 3.2 and
	...
	/**
	 * Webdata! pre-flight event. This runs before Webdata! installs or updates the component. This is our last chance to
	 * tell Webdata! if it should abort the installation.
	 *
	...
	public function preflight($type, $parent)
	{
		...
		// Check the minimum Webdata! version
		if (!empty($this->minimumWebdataVersion) && !version_compare(JVERSION, $this->minimumWebdataVersion, 'ge'))
		{
			$msg = "<p>You need Webdata! $this->minimumWebdataVersion or later to install this component</p>";
		...
		// Check the maximum Webdata! version
		if (!empty($this->maximumWebdataVersion) && !version_compare(JVERSION, $this->maximumWebdataVersion, 'le'))
		{
			$msg = "<p>You need Webdata! $this->maximumWebdataVersion or earlier to install this component</p>";
	...
	/**
	 * Runs after install, update or discover_update. In other words, it executes after Webdata! has finished installing
	...
	public function postflight($type, $parent)
	{
		...
		// Make sure the Webdata! menu structure is correct
		$this->_rebuildMenu();

		// Add post-installation messages on Webdata! 3.2 and later
	...
	public function uninstall($parent)
	{
		...
		// Uninstall post-installation messages on Webdata! 3.2 and later
	...
	/**
	 * Copies the CLI scripts into Webdata!'s cli directory
	 *
	...
	/**
	 * Webdata! 1.6+ bugfix for "Can not build admin menus"
	 */
	 ...
	 * @return bool
	 *
	 * @throws Exception When the Webdata! menu is FUBAR
	 */
	private function _createAdminMenus($parent)
	{
		...
		// We need to insert the menu item as the last child of Webdata!'s menu root node. By default this is the
	...
	/**
	 * Tells Webdata! to rebuild its menu structure to make triple-sure that the Components menu items really do exist
	...
	/**
	 * Adds or updates a post-installation message (PIM) definition for Webdata! 3.2 or later. You can use this in your
	 ...
	 *                        should be shown to the user. @see FOFTemplateUtils::parsePath() for RAD path format. Webdata!
	 ...
	 *
	 * @see                   FOFTemplateUtils::parsePath() for RAD path format. Webdata! will include this file
	...
	/**
	 * Applies the post-installation messages for Webdata! 3.2 or later
	 *
	 * @return void
	 */
	protected function _applyPostInstallationMessages()
	{
		// Make sure it's Webdata! 3.2.0 or later
