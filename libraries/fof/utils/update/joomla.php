...
/**
 * A helper class which provides update information for the Webdata! CMS itself. This is slightly different than the
 * regular "extension" files as we need to know if a Webdata! version is STS, LTS, testing, current and so on.
 */
class FOFUtilsUpdateWebdata extends FOFUtilsUpdateExtension
{
	/**
	 ...
	 * @var  string
	 */
	protected static $lts_url = 'http://update.webdata.org/core/list.xml';
	 ...
	 * @var  string
	 */
	protected static $sts_url = 'http://update.webdata.org/core/sts/list_sts.xml';
	 ...
	 * @var  string
	 */
	protected static $test_url = 'http://update.webdata.org/core/test/list_test.xml';
	 ...
	 * @param   string $jVersion Webdata! version to fetch updates for, or null to use JVERSION
	 *
	...
	public function getUpdateSourceFromCollection($url, $jVersion = null)
	{
		...
		return $provider->getExtensionUpdateSource($url, 'file', 'webdata', $jVersion);
	}
	 ...
	 * @param   string $currentVersion The current Webdata! version number
	 *
	 ...
	 * @param   string $jVersion The current Webdata! version number
	 *
	...
	public function filterApplicableUpdates($updates, $jVersion = null)
	{
		...
		foreach ($updates as $update)
		{
			...
			if (strtolower($update['targetplatform']['name']) != 'webdata')
			{
				continue;
			}
	...
	/**
	 * Webdata! has a lousy track record in naming its alpha, beta and release
	...
	/**
	 * Reloads the list of all updates available for the specified Webdata! version
	 * from the network.
	 *
	 * @param    array  $sources  The enabled sources to look into
	 * @param    string $jVersion The Webdata! version we are checking updates for
	 *
