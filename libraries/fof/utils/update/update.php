...
{
	jimport('webdata.updater.updater');
}

/**
 * A helper Model to interact with Webdata!'s extensions update feature
 */
class FOFUtilsUpdate extends FOFModel
{
	/** @var JUpdater The Webdata! updater object */
...
This email has been automatically sent by a CLI script or Webdata! plugin you, or
...
ENDBLOCK;
	 ...
	 * @param   string  $preferredMethod  Preferred update method: 'webdata' or 'classic'
	 *
	...
	public function findUpdates($force, $preferredMethod = null)
	{
		...
		{
			case 'webdata':
				return $this->findUpdatesWebdata($force);
				break;
	...
	/**
	 * Refreshes the Webdata! update sites for this extension as needed
	 *
	 * @return  void
	 */
	public function refreshUpdateSite()
	{
		// Webdata! 1.5 does not have update sites.
	...
	/**
	 * Get the update method we should use, 'webdata' or 'classic'
	 *
	 * You can defined the preferred update method: 'webdata' uses JUpdater whereas 'classic' handles update caching and
	 * parsing internally. If you are on Webdata! 3.1 or earlier this option is forced to 'classic' since these old
	 * Webdata! versions couldn't handle updates of commercial components correctly (that's why I contributed the fix to
	 * that problem, the extra_query field that's present in Webdata! 3.2 onwards).
	 *
	 * If 'classic' is defined then it will be used in *all* Webdata! versions. It's the most stable method for fetching
	 * update information.
	 *
	 * @param   string  $preferred  Preferred update method. One of 'webdata' or 'classic'.
	 *
	 * @return  string
	 */
	public function getUpdateMethod($preferred = null)
	{
		...
		$validMethods = array('webdata', 'classic');

		if (!in_array($method, $validMethods))
		{
			$method = FOFUtilsConfigHelper::getComponentConfigurationValue($this->component, 'update_method', 'webdata');
		}

		// We can't handle updates using Webdata!'s extensions updater in Webdata! 3.1 and earlier
		if (($method == 'webdata') && version_compare(JVERSION, '3.2.0', 'lt'))
		{
			$method = 'classic';
		}
	 ...
	 * @return  \stdClass|null
	 */
	protected function findUpdatesWebdata($force = false)
	{
		$db = FOFPlatform::getInstance()->getDbo();
		...
		JLoader::import('webdata.updater.update');
	...
	protected function loadUpdatesClassic($force = false)
	{
		...
		if (!$force)
		{
			// Get the cache timeout. On older Webdata! installations it will always default to 6 hours.
			$timeout = 3600 * FOFUtilsConfigHelper::getComponentConfigurationValue('com_installer', 'cachetimeout', '6');
		...
		// Walk through the updates and find the ones compatible with our Webdata! and PHP version
		$compatibleUpdates = array();

		// Get the Webdata! version family (e.g. 2.5)
		$jVersion = JVERSION;
		...
		foreach ($updates as $update)
		{
			...
			// Wrong platform?
			if ($update['targetplatform']['name'] != 'webdata')
			{
				continue;
			}

			// Get the target Webdata! version
			$targetWebdataVersion = $update['targetplatform']['version'];
			$targetVersionParts = explode('.', $targetWebdataVersion);
			$targetVersionShort = $targetVersionParts[0] . '.' . $targetVersionParts[1];

			// The target version MUST be in the same Webdata! branch
			...
			if (version_compare($targetWebdataVersion, JVERSION, 'gt'))
			{
				continue;
			}
	...
	/**
	 * Automatically install the extension update under Webdata! 1.5.5 or later (web) / 3.0 or later (CLI).
	 *
	...
	private function updateComponent()
	{
		...
		{
			return "Extension updates{$errorQualifier}only work with Webdata! $minVersion and later.";
		}
		...
		jimport('webdata.installer.helper');
		...
		if (!$package)
		{
			...
			return "An error occurred while unpacking the file. Please double check your Webdata temp-directory setting in Global Configuration.";
		}
	...
	/**
	 * Downloads the latest update package to Webdata!'s temporary directory
	 *
	...
	public function downloadUpdate()
	{
		...
		if (!$tmp_dest)
		{
			throw new RuntimeException("You must set a non-empty Webdata! temp-directory in Global Configuration before continuing.");
		}

		if (!JFolder::exists($tmp_dest))
		{
			throw new RuntimeException("Webdata!'s temp-directory does not exist. Please set the correct path in Global Configuration before continuing.");
		}
		...
		if (!JFile::write($filename, $data))
		{
			...
			{
				throw new RuntimeException("Webdata!'s temp-directory is not writeable. Please check its permissions or set a different, writeable path in Global Configuration before continuing.");
			}
		}
