...
class FOFTable extends FOFUtilsObject implements JTableInterface
{
	...
	/**
	 * Does the resource support webdata tags?
	 *
	 * @var    boolean
	 */
	...
	public function __construct($table, $key, &$db, $config = array())
	{
		...
		{
			JLoader::import('webdata.access.rules');
		...
		if ($j_query)
		{
			...
			if ($j_query->join)
			{
				foreach ($j_query->join as $join)
				{
					...
					// Webdata doesn't provide any access to the "name" variable, so I have to work with strings...
