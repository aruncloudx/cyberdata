 ...
 *
 * This implements the platform class for Webdata! 2.5 or later
 *
 * @package  FrameworkOnFramework
 * @since    2.1
 */
class FOFIntegrationWebdataPlatform extends FOFPlatform implements FOFPlatformInterface
{
	/**
	...
	{
		$this->name = 'webdata';
		$this->humanReadableName = 'Webdata!';
	...
	public function authorizeAdmin($component)
	{
		if ($this->isBackend())
		{
			// Master access check for the back-end, Webdata! 1.6 style.
    ...
    {
		return FOFDatabaseFactory::getInstance()->getDriver('webdata');
    }
	...
	public function importPlugin($type)
	{
		if (!$this->isCli())
		{
            JLoader::import('webdata.plugin.helper');
	...
	private function &getCacheObject($force = false)
	{
		...
		if (is_null($this->_cache) || $force)
		{
			...
			JLoader::import('webdata.registry.registry');
			$this->_cache = new JRegistry;

			// Try to get data from Webdata!'s cache
	...
	{
		JLoader::import('webdata.user.authentication');
        ...
        // if we're in Webdata 2.5.18+ or 3.2.1+
		...
		if ($response->status == JAuthentication::STATUS_SUCCESS)
		{
			...
			JLoader::import('webdata.user.helper');
    ...
    {
        JLoader::import('webdata.environment.uri');
