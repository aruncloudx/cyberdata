...
class FOFRenderStrapper extends FOFRenderAbstract
{
	...
	public function preRender($view, $task, $input, $config = array())
	{
		...
		if ($platform->isBackend())
		{
			...
			$classes = array(
				'webdata-version-' . $majorVersion,
				'webdata-version-' . $minorVersion,
				$area,
				$option,
				'view-' . $view,
				'layout-' . $layout,
		...
		{
			// @TODO: Remove the frontend Webdata! version classes in FOF 3
			$classes = array(
				'webdata-version-' . $majorVersion,
				'webdata-version-' . $minorVersion,
			);
	...
	protected function loadValidationScript(FOFForm &$form)
	{
		...
		$js = <<<JS
Webdata.submitbutton = function(task)
{
	if (task == 'cancel' || document.formvalidator.isValid(document.id('adminForm')))
	{
		Webdata.submitform(task, document.getElementById('adminForm'));
	}
	else {
		alert('$message');
	}
...
JS;
	...
	protected function renderLinkbar($view, $task, $input, $config = array())
	{
		...
		{
			case 'webdata':
				$this->renderLinkbar_webdata($view, $task, $input);
				break;
	...
	/**
	 * Renders the submenu (link bar) using Webdata!'s style. On Webdata! 2.5 this
	 * is a list of bar separated links, on Webdata! 3 it's a sidebar at the
	 ...
	 * @return  void
	 */
	protected function renderLinkbar_webdata($view, $task, $input, $config = array())
	{
	...
	protected function renderButtons($view, $task, $input, $config = array())
	{
        ...
        FOFPlatform::getInstance()->loadTranslations('webdata');
...
<script type="text/javascript">
	Webdata.orderTable = function() {
		...
		Webdata.tableOrdering(order, dirn);
	};
</script>
...
HTML;
		...
		if ($show_header || $show_filters)
		{
			foreach ($headerFields as $headerField)
			{
				...
				// Under Webdata! < 3.0 we can't have filter-only fields
		...
		if (version_compare(JVERSION, '3.0', 'ge'))
		{
			...
			if ($show_header)
			{
				...
				if (!empty($sortFields))
				{
					...
					$html .= "\t\t" . '<select name="directionTable" id="directionTable" class="input-medium" onchange="Webdata.orderTable()">' . "\n";
					...
					$html .= "\t\t" . '<select name="sortTable" id="sortTable" class="input-medium" onchange="Webdata.orderTable()">' . "\n";
		...
		if ($count = count($items))
		{
			...
			foreach ($items as $i => $item)
			{
				...
				foreach ($fields as $field)
				{
					...
					$labelClass = $field->labelClass ? $field->labelClass : $field->labelclass; // Webdata! 2.5/3.x use different case for the same name
		...
		// Close the wrapper element div on Webdata! 3.0+
		...
		// The id field is required in Webdata! 3 front-end to prevent the pagination limit box from screwing it up. Huh!!
...
JS;
			...
			{
				// Webdata! 2.5 has a conflict with the jQueryUI tooltip, therefore we
				// have to use native Webdata! 2.5 tooltips
				JHtml::_('behavior.tooltip');
