...
class FOFDatabaseQueryOracle extends FOFDatabaseQueryPdo implements FOFDatabaseQueryPreparable, FOFDatabaseQueryLimitable
{
	...
	public function processLimit($query, $limit, $offset = 0)
	{
		...
		if ($limit || $offset)
		{
			$query = "SELECT webdata2.*
		              FROM (
		                  SELECT webdata1.*, ROWNUM AS webdata_db_rownum
		                  FROM (
		                      " . $query . "
		                  ) webdata1
		              ) webdata2";
			...
			if ($limit > 0)
			{
				$query .= ' WHERE webdata2.webdata_db_rownum BETWEEN ' . ($offset + 1) . ' AND ' . ($offset + $limit);
			}
			else
			{
				...
				if ($offset)
				{
					$query .= ' WHERE webdata2.webdata_db_rownum > ' . ($offset + 1);
				}
			}
		}
