...
abstract class JHtmlLinks
{
	...
	public static function linksgroups($groupsOfLinks)
	{
		...
		{
			$layout = new JLayoutFile('webdata.links.groupsopen');
			$html[] = $layout->render('');
			...
			foreach ($groupsOfLinks as $title => $links)
			{
				...
				{
					$layout = new JLayoutFile('webdata.links.groupseparator');
					$html[] = $layout->render($title);
				}

				$layout = new JLayoutFile('webdata.links.groupopen');
				...
				if ($htmlLinks != '')
				{
					...
					$layout = new JLayoutFile('webdata.links.groupclose');
					$html[] = $layout->render('');
				}
			}

			$layout = new JLayoutFile('webdata.links.groupsclose');
			$html[] = $layout->render('');
		}
	...
	public static function link($link)
	{
		...
		$layout = new JLayoutFile('webdata.links.link');
