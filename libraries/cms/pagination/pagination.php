...
/**
 * Pagination Class. Provides a common interface for content pagination for the Webdata! CMS.
 *
 * @since  1.5
 */
class JPagination
{
	...
	public function getPagesLinks()
	{
		...
		if (file_exists($chromePath))
		{
			...
			{
				JLog::add(
					'pagination_item_active and pagination_item_inactive are deprecated. Use the layout webdata.pagination.link instead.',
					JLog::WARNING,
					'deprecated'
				);
			...
			{
				JLog::add('pagination_list_render is deprecated. Use the layout webdata.pagination.list instead.', JLog::WARNING, 'deprecated');
				$listOverride = true;
			}
		}
	 ...
	 * @since   3.3
	 */
	public function getPaginationLinks($layoutId = 'webdata.pagination.links', $options = array())
	{
		// Allow to receive a null layout
		$layoutId = $layoutId === null ? 'webdata.pagination.links' : $layoutId;
	...
	public function getListFooter()
	{
		...
		if (file_exists($chromePath))
		{
			...
			{
				JLog::add('pagination_list_footer is deprecated. Use the layout webdata.pagination.links instead.', JLog::WARNING, 'deprecated');
	...
	public function getLimitBox()
	{
		...
		if ($this->app->isClient('administrator'))
		{
			$html = JHtml::_(
				...
				$limits,
				$this->prefix . 'limit',
				'class="inputbox input-mini" size="1" onchange="Webdata.submitform();"',
				'value',
				'text',
				$selected
			);
	...
	{
		return JLayoutHelper::render('webdata.pagination.list', array('list' => $list));
	}
	 ...
	 *
	 * @since   1.5
	 * @note    As of 4.0 this method will proxy to `JLayoutHelper::render('webdata.pagination.link', ['data' => $item, 'active' => true])`
	 */
	protected function _item_active(JPaginationObject $item)
	{
		...
		if ($this->app->isClient('administrator'))
		{
			...
			. 'limitstart.value=' . ($item->base > 0 ? $item->base : '0') . '; Webdata.submitform();return false;">' . $item->text . '</a>';
		}
		else
		{
	 ...
	 *
	 * @since   1.5
	 * @note    As of 4.0 this method will proxy to `JLayoutHelper::render('webdata.pagination.link', ['data' => $item, 'active' => false])`
	 */
