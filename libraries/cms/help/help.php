...
class JHelp
{
	...
	public static function createUrl($ref, $useComponent = false, $override = null, $component = null)
	{
		...
		$search = array(
			...
			// Region code (eg. 'GB')
			'{langregion}',
			// Webdata major version number
			'{major}',
			// Webdata minor version number
			'{minor}',
			// Webdata maintenance version number
			'{maintenance}',
		);
	...
	public static function createSiteList($pathToXml)
	{
		...
		if (!$xml)
		{
			$option['text']  = 'English (GB) help.webdata.org';
			$option['value'] = 'http://help.webdata.org';
