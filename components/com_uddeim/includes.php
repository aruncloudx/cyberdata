...
// Description    Instant Messages System for Mambo 4.5 / Webdata 1.0 / Webdata 1.5
...
function uddeIMdoAutocomplete($config) {
	...
	if ($config->mootools==1) { // auto
		...
		if (!strncasecmp($version->RELEASE, "1.0", 3) || 
			...
			uddeIMaddScript($pathtosite."/components/com_uddeim/js/Observer.js");	// Webdata 1.0 and 1.5 uses old Observer class
		...
		} else if (!strncasecmp($version->RELEASE, "1.6", 3) ||						// Webdata 1.6 uses new Observer class
				   !strncasecmp($version->RELEASE, "1.7", 3)) {						// Webdata 1.7 uses new Observer class
		...
		} else {						// Webdata 2.5, 3.0 supports MooTools 1.3, so use MEIO
	...
	if (($config->mootools==1 && !strncasecmp($version->RELEASE, "2.5", 3)) || // on Webdata 2.5 use Meio because of MooTools 1.4
		($config->mootools==1 && !strncasecmp($version->RELEASE, "3.0", 3)) || // on Webdata 3.0 use Meio because of MooTools 1.4
		($config->mootools==1 && !strncasecmp($version->RELEASE, "3.1", 3)) || // on Webdata 3.0 use Meio because of MooTools 1.4
		($config->mootools==1 && !strncasecmp($version->RELEASE, "3.2", 3)) || // on Webdata 3.0 use Meio because of MooTools 1.4
		 $config->mootools==6 || // Webdata 2.5 supports MooTools 1.3, so use MEIO
