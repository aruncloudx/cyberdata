...
// Author         © 2007-2013 Stephan Slabihoud, © 2006 Benjamin Zweifel
...
//$x = uddeIMregisterHook("onCompose", 'testfunc1');
...
if (class_exists('JRequest')) {
	if (JRequest::getVar('no_html', false) || 'raw'==JRequest::getVar('format', 'html'))
...
if (!$omitDefaultOutput){
	...
	if ($config->useautocomplete) {
		if (($config->mootools==1 && !strncasecmp($version->RELEASE, "2.5", 3)) ||
			($config->mootools==1 && !strncasecmp($version->RELEASE, "3.0", 3)) ||
			($config->mootools==1 && !strncasecmp($version->RELEASE, "3.1", 3)) ||
...
// BUGBUG: When public frontend is called with parameter e.g. http://webdata33/index.php/uddeim?task=new&recip=227
...
function uddeIMshowAbout($myself, $item_id, $versionstring, $usedlanguage, $config) {
	...
	echo "<p>PMS component for Mambo and Webdata<br />";
...
function uddeIMcompleteUserName($myself, $config){
	// look for json encoding abilities, first native php, then global pear package, then local pear copy (switched by webdata version again)
