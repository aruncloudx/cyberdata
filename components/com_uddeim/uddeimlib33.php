...
jimport('webdata.utilities.date');
jimport('webdata.utilities.utility');
...
// The timezone support is completely messed up in Webdata because of several API changes.
...
if (!class_exists('uddeIMmosPageNav')) {
	jimport('webdata.html.pagination');
...
function uddeIMisWritable($file, $forcenoftp=false) {
	$options = array();
	$ret = false;
	if (class_exists('JFactory')) {		// Webdata 1.5?
		$config = JFactory::getConfig();
		$options = array(
